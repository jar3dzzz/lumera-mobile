...
export default function AnimalEditFormComponent({
  ...
  const { setAnimals } = useRancho();

  // Determine type selector values
  const isPresetType = ['VACA', 'BECERRO', 'TORO'].includes(animal.animalType);

  // Form states
  const [nombre, setNombre] = useState<string>(animal.nombre || '');
  const [edad, setEdad] = useState<string>(animal.edad !== undefined ? String(animal.edad) : '');
  const [fechaNacimiento, setFechaNacimiento] = useState<string>(animal.fechaNacimiento || '');
  const [animalType, setAnimalType] = useState<string>(isPresetType ? animal.animalType : 'OTRO');
  const [customType, setCustomType] = useState<string>(isPresetType ? '' : animal.animalType);
  const [color, setColor] = useState<string>(animal.Color || '');
  const [arete, setArete] = useState<string>(animal.Arete ? String(animal.Arete) : '');
  const [pierna1, setPierna1] = useState<string>(animal.Pierna1 || '');
  const [pieran2, setPieran2] = useState<string>(animal.Pieran2 ? String(animal.Pieran2) : '');
  const [serie1, setSerie1] = useState<string>(animal.Serie1 || '');
  const [serie2, setSerie2] = useState<string>(animal.Serie2 ? String(animal.Serie2) : '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updatedAnimal: Core.Animal = {
        ...animal, // Preserve id, created_by, created_at
      ...
      // Updating the mock animal in current context list
      ...
      Alert.alert('Error', err.message || 'Ocurrió un error al actualizar el animal.');
    } finally {
      setLoading(false);
    }
  };
